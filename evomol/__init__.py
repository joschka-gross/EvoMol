...
from .evaluation import (
    EvaluationStrategy,
    GenericFunctionEvaluationStrategy,
    ...
    ZincNormalizedPLogPEvaluationStrategy,
    LinearCombinationEvaluationStrategy,
    ProductSigmLinEvaluationStrategy,
    ProductEvaluationStrategy,
    SigmLinWrapperEvaluationStrategy,
    GaussianWrapperEvaluationStrategy,
    EvaluationStrategyComposant,
    OppositeWrapperEvaluationStrategy,
    IsomerGuacaMolEvaluationStrategy,
    MeanEvaluationStrategyComposite,
    OneMinusWrapperEvaluationStrategy,
    NPerturbationsEvaluationStrategy,
    AbsoluteDifferenceEvaluationStrategy,
    SillyWalksEvaluationStrategy,
...
from .stopcriterion import (
    MultipleStopCriterionsStrategy,
    FileStopCriterion,
    KStepsStopCriterionStrategy,
...
def _is_describing_custom_function(param_eval):
    ...
    :return:
    """
    return callable(param_eval) or (
        isinstance(param_eval, tuple) and callable(param_eval[0])
    )
...
def _is_describing_implemented_function(param_eval):
    ...
    return (
        ...
        or param_eval.startswith("isomer")
        or param_eval.startswith("rediscovery")
    )
...
def _build_evaluation_strategy_from_custom_function(obj_fun_param):
    ...
    if isinstance(obj_fun_param, tuple):
        strat = GenericFunctionEvaluationStrategy(
            evaluation_function=obj_fun_param[0], function_name=obj_fun_param[1]
        )
...
def _build_evaluation_strategy_from_implemented_function(
    param_eval,
    ...
    if param_eval == "qed":
        ...
        or param_eval == "lumo"
        or param_eval == "gap"
        or param_eval == "homo-1"
    ):
        strat = OPTEvaluationStrategy(
            param_eval,
        ...
        strat = EntropyContribEvaluationStrategy(
            explicit_search_parameters_dict["n_max_desc"],
...
def _build_evaluation_strategy_from_single_objective(
    param_eval,
    explicit_IO_parameters_dict,
    explicit_search_parameters_dict,
    shared_last_DFT_computation,
    ...
    # Parameter describes an implemented function
        ...
        return _build_evaluation_strategy_from_implemented_function(
            param_eval,
            explicit_IO_parameters_dict,
            explicit_search_parameters_dict,
            shared_last_DFT_computation,
...
def _build_evaluation_strategy_from_multi_objective(
    param_eval,
    explicit_IO_parameters_dict,
    explicit_search_parameters_dict,
    shared_last_DFT_computation,
    ...
    if param_eval["type"] in [
        ...
        functions_desc = (
            param_eval["functions"]
            if "functions" in param_eval and param_eval["functions"]
            else [param_eval["function"]]
        )
        ...
        for function_desc in functions_desc:
            ...
            if _is_describing_multi_objective_function(function_desc):
                evaluation_strategies.append(
                    _build_evaluation_strategy_from_multi_objective(
                        function_desc,
                        explicit_IO_parameters_dict,
                        explicit_search_parameters_dict,
                        shared_last_DFT_computation,
                ...
                evaluation_strategies.append(
                    _build_evaluation_strategy_from_single_objective(
                        function_desc,
                        explicit_IO_parameters_dict,
                        explicit_search_parameters_dict,
                        shared_last_DFT_computation,
        ...
        if param_eval["type"] == "linear_combination":
            return LinearCombinationEvaluationStrategy(
                evaluation_strategies, coefs=param_eval["coef"]
            )
        elif param_eval["type"] == "product":
            return ProductEvaluationStrategy(evaluation_strategies)
        elif param_eval["type"] == "sigm_lin":
            return SigmLinWrapperEvaluationStrategy(
                evaluation_strategies,
                a=param_eval["a"],
                b=param_eval["b"],
                l=param_eval["lambda"],
            ...
            return ProductSigmLinEvaluationStrategy(
                evaluation_strategies,
                a=param_eval["a"],
                b=param_eval["b"],
                l=param_eval["lambda"],
            ...
            return GaussianWrapperEvaluationStrategy(
                evaluation_strategies,
                mu=param_eval["mu"],
                sigma=param_eval["sigma"],
                normalize=(
                    param_eval["normalize"] if "normalize" in param_eval else False
                ),
            )
...
def _parse_objective_function_strategy(
    parameters_dict, explicit_IO_parameters_dict, explicit_search_parameters_dict
):
    """
    ...
    # Parameter is a multi-objective function
        ...
        eval_strat = _build_evaluation_strategy_from_multi_objective(
            param_eval,
            explicit_IO_parameters_dict,
            explicit_search_parameters_dict,
            shared_last_DFT_computation,
    ...
    # Parameter is a single objective function
        ...
        eval_strat = _build_evaluation_strategy_from_single_objective(
            param_eval,
            explicit_IO_parameters_dict,
            explicit_search_parameters_dict,
            shared_last_DFT_computation,
...
def _parse_action_space(parameters_dict):
    ...
    input_param_action_space = (
        parameters_dict["action_space_parameters"]
        if "action_space_parameters" in parameters_dict
        else {}
    )

    explicit_action_space_parameters = {
        "atoms": (
            input_param_action_space["atoms"]
            if "atoms" in input_param_action_space
            else "C,N,O,F,P,S,Cl,Br"
        ),
        "max_heavy_atoms": (
            input_param_action_space["max_heavy_atoms"]
            if "max_heavy_atoms" in input_param_action_space
            else 38
        ),
        "substitution": (
            input_param_action_space["substitution"]
            if "substitution" in input_param_action_space
            else True
        ),
        "append_atom": (
            input_param_action_space["append_atom"]
            if "append_atom" in input_param_action_space
            else True
        ),
        "remove_atom": (
            input_param_action_space["remove_atom"]
            if "remove_atom" in input_param_action_space
            else True
        ),
        "change_bond": (
            input_param_action_space["change_bond"]
            if "change_bond" in input_param_action_space
            else True
        ),
        "change_bond_prevent_breaking_creating_bonds": (
            input_param_action_space["change_bond_prevent_breaking_creating_bonds"]
            if "change_bond_prevent_breaking_creating_bonds" in input_param_action_space
            else False
        ),
        "cut_insert": (
            input_param_action_space["cut_insert"]
            if "cut_insert" in input_param_action_space
            else True
        ),
        "move_group": (
            input_param_action_space["move_group"]
            if "move_group" in input_param_action_space
            else True
        ),
        "remove_group": (
            input_param_action_space["remove_group"]
            if "remove_group" in input_param_action_space
            else False
        ),
        "remove_group_only_remove_smallest_group": (
            input_param_action_space["remove_group_only_remove_smallest_group"]
            if "remove_group_only_remove_smallest_group" in input_param_action_space
            else True
        ),
        "use_rd_filters": (
            input_param_action_space["use_rd_filters"]
            if "use_rd_filters" in input_param_action_space
            else False
        ),
        "sillywalks_threshold": (
            input_param_action_space["sillywalks_threshold"]
            if "sillywalks_threshold" in input_param_action_space
            else 1
        ),
        "sascore_threshold": (
            input_param_action_space["sascore_threshold"]
            if "sascore_threshold" in input_param_action_space
            else float("inf")
        ),
        "custom_filter_function": (
            input_param_action_space["custom_filter_function"]
            if "custom_filter_function" in input_param_action_space
            else None
        ),
        "sulfur_valence": (
            input_param_action_space["sulfur_valence"]
            if "sulfur_valence" in input_param_action_space
            else 6
        ),
    }
    ...
    action_spaces, action_spaces_parameters = generic_action_space(
        atom_symbols_list=symbols_list,
    ...
    for parameter in input_param_action_space:
        if parameter not in explicit_action_space_parameters:
            raise RuntimeWarning(
                "Unrecognized parameter in 'action_space_parameters' : "
                + str(parameter)
            )
...
def _parse_mutation_parameters(
    ...
    mutation_strategy = KRandomGraphOpsImprovingMutationStrategy(
        k=explicit_search_parameters["mutation_max_depth"],
        max_n_try=explicit_search_parameters["mutation_find_improver_tries"],
        evaluation_strategy=evaluation_strategy,
        action_spaces=action_spaces,
        action_spaces_parameters=action_spaces_parameters,
        problem_type=explicit_search_parameters["problem_type"],
        quality_filter=search_space_parameters["use_rd_filters"],
        silly_molecules_fp_threshold=search_space_parameters["sillywalks_threshold"],
        silly_molecules_db=explicit_IO_parameters["silly_molecules_reference_db_path"],
        sascore_threshold=search_space_parameters["sascore_threshold"],
        neighbour_gen_strategy=explicit_search_parameters[
            "neighbour_generation_strategy"
        ],
        custom_filter_function=search_space_parameters["custom_filter_function"],
    )
...
def _extract_explicit_search_parameters(parameters_dict):
    ...
    input_search_parameters = (
        parameters_dict["optimization_parameters"]
        if "optimization_parameters" in parameters_dict
        else {}
    )

    explicit_search_parameters = {
        "problem_type": (
            input_search_parameters["problem_type"]
            if "problem_type" in input_search_parameters
            else "max"
        ),
        "pop_max_size": (
            input_search_parameters["pop_max_size"]
            if "pop_max_size" in input_search_parameters
            else 1000
        ),
        "k_to_replace": (
            input_search_parameters["k_to_replace"]
            if "k_to_replace" in input_search_parameters
            else 10
        ),
        "selection": (
            input_search_parameters["selection"]
            if "selection" in input_search_parameters
            else "best"
        ),
        "max_steps": (
            input_search_parameters["max_steps"]
            if "max_steps" in input_search_parameters
            else 1500
        ),
        "max_obj_calls": (
            input_search_parameters["max_obj_calls"]
            if "max_obj_calls" in input_search_parameters
            else float("inf")
        ),
        "stop_kth_score_value": (
            input_search_parameters["stop_kth_score_value"]
            if "stop_kth_score_value" in input_search_parameters
            else None
        ),
        "mutable_init_pop": (
            input_search_parameters["mutable_init_pop"]
            if "mutable_init_pop" in input_search_parameters
            else True
        ),
        "guacamol_init_top_100": (
            input_search_parameters["guacamol_init_top_100"]
            if "guacamol_init_top_100" in input_search_parameters
            else False
        ),
        "mutation_max_depth": (
            input_search_parameters["mutation_max_depth"]
            if "mutation_max_depth" in input_search_parameters
            else 2
        ),
        "mutation_find_improver_tries": (
            input_search_parameters["mutation_find_improver_tries"]
            if "mutation_find_improver_tries" in input_search_parameters
            else 50
        ),
        "n_max_desc": (
            input_search_parameters["n_max_desc"]
            if "n_max_desc" in input_search_parameters
            else 3000000
        ),
        "shuffle_init_pop": (
            input_search_parameters["shuffle_init_pop"]
            if "shuffle_init_pop" in input_search_parameters
            else False
        ),
        "neighbour_generation_strategy": (
            input_search_parameters["neighbour_generation_strategy"]
            if "neighbour_generation_strategy" in input_search_parameters
            else RandomActionTypeSelectionStrategy()
        ),
    }
    ...
    for parameter in input_search_parameters:
        if parameter not in explicit_search_parameters:
            raise RuntimeWarning(
                "Unrecognized parameter in 'search_parameters' : " + str(parameter)
            )
...
def _extract_explicit_IO_parameters(parameters_dict):
    ...
    input_IO_parameters = (
        parameters_dict["io_parameters"] if "io_parameters" in parameters_dict else {}
    )
    explicit_IO_parameters = {
        "model_path": (
            input_IO_parameters["model_path"]
            if "model_path" in input_IO_parameters
            else "EvoMol_model/"
        ),
        "smiles_list_init_path": (
            input_IO_parameters["smiles_list_init_path"]
            if "smiles_list_init_path" in input_IO_parameters
            else None
        ),
        "smiles_list_init": (
            input_IO_parameters["smiles_list_init"]
            if "smiles_list_init" in input_IO_parameters
            else None
        ),
        "external_tabu_list": (
            input_IO_parameters["external_tabu_list"]
            if "external_tabu_list" in input_IO_parameters
            else None
        ),
        "save_n_steps": (
            input_IO_parameters["save_n_steps"]
            if "save_n_steps" in input_IO_parameters
            else 100
        ),
        "print_n_steps": (
            input_IO_parameters["print_n_steps"]
            if "print_n_steps" in input_IO_parameters
            else 1
        ),
        "dft_working_dir": (
            input_IO_parameters["dft_working_dir"]
            if "dft_working_dir" in input_IO_parameters
            else "/tmp/"
        ),
        "dft_cache_files": (
            input_IO_parameters["dft_cache_files"]
            if "dft_cache_files" in input_IO_parameters
            else []
        ),
        "dft_MM_program": (
            input_IO_parameters["dft_MM_program"]
            if "dft_MM_program" in input_IO_parameters
            else "obabel_mmff94"
        ),
        "dft_base": (
            input_IO_parameters["dft_base"]
            if "dft_base" in input_IO_parameters
            else "3-21G*"
        ),
        "dft_method": (
            input_IO_parameters["dft_method"]
            if "dft_method" in input_IO_parameters
            else "B3LYP"
        ),
        "dft_n_jobs": (
            input_IO_parameters["dft_n_jobs"]
            if "dft_n_jobs" in input_IO_parameters
            else 1
        ),
        "dft_mem_mb": (
            input_IO_parameters["dft_mem_mb"]
            if "dft_mem_mb" in input_IO_parameters
            else 512
        ),
        "record_history": (
            input_IO_parameters["record_history"]
            if "record_history" in input_IO_parameters
            else False
        ),
        "record_all_generated_individuals": (
            input_IO_parameters["record_all_generated_individuals"]
            if "record_all_generated_individuals" in input_IO_parameters
            else False
        ),
        "evaluation_strategy_parameters": (
            input_IO_parameters["evaluation_strategy_parameters"]
            if "evaluation_strategy_parameters" in input_IO_parameters
            else None
        ),
        "silly_molecules_reference_db_path": (
            input_IO_parameters["silly_molecules_reference_db_path"]
            if "silly_molecules_reference_db_path" in input_IO_parameters
            else None
        ),
    }
    ...
    for parameter in input_IO_parameters:
        if parameter not in explicit_IO_parameters:
            raise RuntimeWarning(
                "Unrecognized parameter in 'io_parameters' : " + str(parameter)
            )

    return explicit_IO_parameters


def _parse_stop_criterion_strategy(
    explicit_search_parameters_dict, explicit_IO_parameters_dict
):
    """
    ...
    criteria = [
        ...
        KObjFunCallsFunctionStopCriterion(
            explicit_search_parameters_dict["max_obj_calls"]
        ),
        FileStopCriterion(
            join(explicit_IO_parameters_dict["model_path"], "stop_execution")
        ),
    ]
...
def build_instance(
    ...
    pop_alg = PopAlg(
        ...
        record_all_generated_individuals=explicit_IO_parameters_dict[
            "record_all_generated_individuals"
        ],
        ...
        evaluation_strategy_parameters=explicit_IO_parameters_dict[
            "evaluation_strategy_parameters"
        ],
        sulfur_valence=explicit_action_space_parameters["sulfur_valence"],
        kth_score_to_record=(
            explicit_search_parameters_dict["stop_kth_score_value"][0]
            if explicit_search_parameters_dict["stop_kth_score_value"] is not None
            else 1
        ),
    )
    ...
    if not is_or_contains_undefined_GuacaMol_evaluation_strategy(evaluation_strategy):
        ...
        if explicit_IO_parameters_dict["smiles_list_init"] is not None:
            pop_alg.load_pop_from_smiles_list(
                explicit_IO_parameters_dict["smiles_list_init"]
            )
        ...
        # Reading smiles list from the file pointed by "smiles_list_init_path" attribute
            ...
            # Initialization of the population from the specified file
                ...
                pop_alg.load_pop_from_smiles_list(
                    smiles_list=_read_smiles_list_from_file(
                        explicit_IO_parameters_dict["smiles_list_init_path"]
                    ),
                    atom_mutability=explicit_search_parameters_dict["mutable_init_pop"],
                )
...
def get_objective_function_instance(parameters_dict):
    ...
    return _parse_objective_function_strategy(
        parameters_dict,
...
def run_model(parameters_dict):
    ...
    action_spaces, action_spaces_parameters, explicit_action_space_parameters = (
        _parse_action_space(parameters_dict)
    )
    ...
    if is_or_contains_undefined_GuacaMol_evaluation_strategy(evaluation_strategy):
        ...
        assess_goal_directed_generation(
            ...
            json_output_file=join(
                explicit_IO_parameters_dict["model_path"], "output_GuacaMol.json"
            ),
